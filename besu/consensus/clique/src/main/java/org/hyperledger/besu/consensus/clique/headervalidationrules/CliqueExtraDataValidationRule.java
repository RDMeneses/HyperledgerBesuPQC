...
public class CliqueExtraDataValidationRule implements AttachedBlockHeaderValidationRule {
  ...
  private boolean extraDataIsValid(
    ...
    final Address proposer1 = cliqueExtraData.getProposerAddress1();
    final Address proposer2 = cliqueExtraData.getProposerAddress2();

    if (!expectedValidators.contains(proposer1)) {
      LOG.info("Invalid block header: Proposer 1 sealing block is not a member of the signers.");
      return false;
    }

    if (!expectedValidators.contains(proposer2)) {
      LOG.info("Invalid block header: Proposer 2 sealing block is not a member of the signers.");
      return false;
    }
