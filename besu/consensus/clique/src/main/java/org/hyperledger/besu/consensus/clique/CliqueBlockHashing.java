...
public class CliqueBlockHashing {
  ...
  /**
   * Recovers the proposer's Address from the proposer seal.
   *
   ...
   * @param sealIndex which seal to recover (1 for the first, 2 for the second)
   * @return the proposer address
   */
  public static Address recoverProposerAddress(
      final BlockHeader header, final CliqueExtraData cliqueExtraData, final int sealIndex) {
    final Optional<SECPSignature> sealOpt =
        sealIndex == 1 ? cliqueExtraData.getProposerSeal1() : cliqueExtraData.getProposerSeal2();

    if (!sealOpt.isPresent()) {
      if (header.getNumber() == BlockHeader.GENESIS_BLOCK_NUMBER) {
        return Address.ZERO;
      }
      throw new IllegalArgumentException("CliqueExtraData missing seal " + sealIndex);
    }
    // hash over header with both seals stripped
    final Hash proposerHash = calculateDataHashForProposerSeal(header, cliqueExtraData);
    return Util.signatureToAddress(sealOpt.get(), proposerHash);
  }
  ...
  private static Bytes encodeExtraDataWithoutProposerSeal(final CliqueExtraData cliqueExtraData) {
    ...
    return extraDataBytes.slice(0, extraDataBytes.size() - 2 * SECPSignature.BYTES_REQUIRED);
  }
